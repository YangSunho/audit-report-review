...
// 반기·분기 보고서는 "제33(당)기 반기말:" · "제 32(전)기 기말:" 처럼
// 기수 뒤에 반기/분기 표기와 콜론이 붙는다. 이것을 흡수하지 못해 연차보고서만
// 기수가 잡히고 반기보고서는 대상기간이 통째로 비었다.
const TERM = String.raw`제\s*(\d{1,3})\s*(?:\(\s*[당전]\s*\))?\s*기\s*(?:\(\s*[당전]\s*\))?\s*(?:반기|분기)?\s*(?:기)?\s*(?:말)?\s*[:：]?`;
